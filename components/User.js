...
const User = ({ item }) => {
    ...
    const fetchFriendRequests = async () => {
      try {
        const response = await fetch(
          `http://192.168.2.174:8000/friend-requests/sent/${userId}`
        );
  ...
  useEffect(() => {
    const fetchUserFriends = async () => {
      try {
        const response = await fetch(`http://192.168.2.174:8000/friends/${userId}`);
  ...
  const sendFriendRequest = async (currentUserId, selectedUserId) => {
    try {
      const response = await fetch("http://192.168.2.174:8000/friend-request", {
        method: "POST",
        headers: {
